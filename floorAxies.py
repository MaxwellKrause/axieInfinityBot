...
image = 2   #1 if you want to load images 2 to block images (faster)

password = "WITHHELD"   #PASSWORD for ronin wallet **withheld for obvious reasons**
...
options.add_argument(r"user-data-dir=/home/max/.config/google-chrome/Default")  #path to chrome profile containing ronin extension, privacy pass, metamask
...
def bot(desiredPrice, sort):
    try: #initiates the bot (login and pull up ronin + market)
        ...
        driver.refresh()
        time.sleep(1)
        driver.find_element_by_id("password-input").send_keys(password + Keys.RETURN)
        time.sleep(1)
        ...
        pass
    playsound.playsound('./requestAixieBot/Notification.wav', False)
    loop = True
    driver.switch_to.window(market)
    time.sleep(5000)
    while(loop):
        try:
            ...
            for i in range(10): #goes through recieved axies
